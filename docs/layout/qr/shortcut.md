# 键盘快捷键

| 功能键命令分配 |                                                              |
| -------------- | ------------------------------------------------------------ |
| **按键**       | **描述**                                                     |
| F1             | 打开帮助（上下文相关）                                       |
| F2             | 添加布线                                                     |
| F3             | 动态布线                                                     |
| F4             | 切换层对                                                     |
| F5             | 选择管脚对                                                   |
| F6             | 选择网络                                                     |
| F7             | 自动布线选中✅对象                                             |
| **编辑操作**   |                                                              |
| **快捷键**     | **描述**                                                     |
| End            | 重绘                                                         |
| Ctrl+D         |                                                              |
| 小键盘 (1)     |                                                              |
| Delete         | 取消选中✅对象的布线                                           |
| Escape         | 取消命令                                                     |
| Ctrl+H         | 高亮对象                                                     |
| Ctrl+U         | 移除当前对象的高亮                                           |
| **选择操作**   |                                                              |
| **快捷键**     | **描述**                                                     |
| Ctrl+Alt+F     | 显示选择过滤器                                               |
| 点击🖱️           | 选择对象                                                     |
| 空格键         |                                                              |
| XP             | 切换拾取模式。默认为“按线宽拾取”。``“按拾取半径拾取”有助于选择小线段。 |
| Ctrl+A         | 根据选择过滤器选择所有设计对象                               |
| 点击🖱️+拖动      | 开始区域选择                                                 |
| Tab            | 在拾取半径内循环选择相邻对象                                 |
| Ctrl+点击🖱️      | 切换对象选择或多选对象                                       |
| **放置操作**   |                                                              |
| **快捷键**     | **描述**                                                     |
| Ctrl+E         | 移动选中✅对象                                                 |
| 点击🖱️+拖动      |                                                              |
| Tab            | 旋转元件（在元件移动模式下）                                 |
| Ctrl+R         | 将选中✅元件旋转 90 度                                           |
| Ctrl+I         | 旋转选中✅对象                                                 |
| Ctrl+F         | 将对象翻转到板子的另一面                                     |
| Ctrl+L         | 对齐元件                                                     |
| **文件操作**   |                                                              |
| **快捷键**     | **描述**                                                     |
| Ctrl+N         | 创建新设计文件（空白）                                       |
| Ctrl+O         | 打开设计文件                                                 |
| Ctrl+S         | 保存设计文件                                                 |
|                |                                                              |

| **平移、缩放与导航**      |                                              |
| ------------------------- | -------------------------------------------- |
| **快捷键**                | **描述**                                     |
| Home                      | 缩放到板框，使其最大化适配工作区             |
| 小键盘 (7)                |                                              |
| Ctrl+B                    |                                              |
| Ctrl+Alt+E                | 缩放到设计中的所有对象，使其最大化适配工作区 |
| 中键+拖动                 | 从中心缩放。向上拖动放大，向下拖动缩小       |
| 小键盘 (5)（Numlock 开启） |                                              |
| Shift+中键+拖动           | 从角落缩放。向上拖动放大，向下拖动缩小       |
| 小键盘 (.)（仅缩放模式）  |                                              |
| Alt+Z                     | 缩放到选中✅对象，使其最大化适配工作区         |
| Ctrl+W                    | 切换缩放模式开/关                            |
| 点击🖱️                      | 在鼠标位置放大（缩放模式）                   |
| Page Up                   |                                              |
| 空格键                    |                                              |
| 小键盘 (9)                |                                              |
| **右键**点击🖱️                  | 在鼠标位置缩小（缩放模式）                   |
| Page Down                 |                                              |
| 小键盘 (3)                |                                              |
| Page Up                   | 在鼠标位置放大                               |
| 小键盘 (9)                |                                              |
| Ctrl+滚轮向前             |                                              |
| Page Down                 | 在鼠标位置缩小                               |
| 小键盘 (3)                |                                              |
| Ctrl+滚轮向后             |                                              |
| 向下箭头                  | 将鼠标向下移动一个设计格栅                   |
| 向上箭头                  | 将鼠标向上移动一个设计格栅                   |
| 向左箭头                  | 将鼠标向左移动一个设计格栅                   |
| 向右箭头                  | 将鼠标向右移动一个设计格栅                   |
| 中键点击🖱️                  | 平移视图以将鼠标位置居中                     |
| 小键盘 (0)                |                                              |
| Insert                    |                                              |
| Alt+中键+拖动             | 动态平移                                     |
| 滚轮向后                  | 向下平移一行                                 |
| 滚轮向前                  | 向上平移一行                                 |
| Shift+滚轮向后            | 向右平移一行                                 |
| Shift+滚轮向前            | 向左平移一行                                 |
| Ctrl+Alt+滚轮向后         | 向下平移一个像素                             |
| Ctrl+Alt+滚轮向前         | 向上平移一个像素                             |
| Alt+Shift+滚轮向后        | 向右平移一个像素                             |
| Alt+Shift+滚轮向前        | 向左平移一个像素                             |
| 小键盘 (2)                | 向下平移半屏高度                             |
| 小键盘 (4)                | 向左平移半屏高度                             |
| 小键盘 (6)                | 向右平移半屏高度                             |

| **视图操作**   |                                                              |
| -------------- | ------------------------------------------------------------ |
| **快捷键**     | **描述**                                                     |
| Alt+B          | 显示底视图（镜像设计视图）                                   |
| Alt+N          | 显示下一个视图                                               |
| Alt+P          | 显示上一个视图                                               |
| **布线操作**   |                                                              |
| **快捷键**     | **描述**                                                     |
| 点击🖱️           | 添加拐角（交互式布线）                                       |
| 空格键         |                                                              |
| 点击🖱️           | 添加定位点                                                   |
| Shift+点击🖱️     | 添加过孔（交互式布线）                                       |
| Ctrl+Alt+J     | 添加跳线（交互式布线）                                       |
| Backspace      | 移除布线的最后一个拐角或 2D 线的最后一个拐角（多边形或路径绘制模式中） |
| Shift+S        | 拉伸                                                         |
| Enter          | 完成布线                                                     |
| 点击🖱️           | 完成（交互式布线），当鼠标位于有效完成点时                   |
| 双击           | 开始布线未布线的路径。当鼠标接近有效完成点时也可完成布线。   |
| Ctrl+点击🖱️      | 结束（交互式布线）                                           |
| Ctrl+M         | 运行长度最小化                                               |
| Ctrl+Page Down | 快速测量/重置 DxDy                                            |
| Ctrl+Page Up   | 在鼠标位置显示布线长度                                       |
| Ctrl+G         | 创建组合                                                     |
| Ctrl+J         | 布线环路                                                     |
| Ctrl+K         | 创建簇                                                       |
| Ctrl+T         | 显示泪滴特性                                                 |

| **总线布线操作**     |                                                            |
| -------------------- | ---------------------------------------------------------- |
| **快捷键**           | **描述**                                                   |
| Ctrl+Alt+B           | 总线布线                                                   |
| Alt+点击🖱️             | 添加拐角到引导布线                                         |
| Alt+Shift+点击🖱️       | 添加过孔到引导布线                                         |
| Ctrl+Tab             | 循环切换过孔模式                                           |
| Ctrl+Alt+点击🖱️        | 结束引导                                                   |
| Tab                  | 下一个引导                                                 |
| Shift+Tab            | 上一个引导                                                 |
| **打开菜单与对话框** |                                                            |
| **快捷键**           | **描述**                                                   |
| **右键**点击🖱️             | 打开弹出菜单                                               |
| 双击                 | 打开选中✅对象的属性对话框                                   |
| Alt+Enter            |                                                            |
| Ctrl+Q               |                                                            |
| Ctrl+Enter           | 打开选项对话框                                             |
| Ctrl+Alt+C           | 打开显示颜色设置对话框                                     |
| Ctrl+Alt+D           | 打开选项对话框的设计页面                                   |
| Ctrl+Alt+G           | 打开选项对话框的全局/常规页面                              |
| Ctrl+Alt+S           | 打开状态窗口                                               |
| Ctrl+Alt+N           | 打开查看网络对话框                                         |
| **鼠标按键替代**     |                                                            |
| **快捷键**           | **描述**                                                   |
| M                    | 打开当前模式的弹出菜单（同**右键**点击🖱️）                       |
| 空格键               | 在当前鼠标位置执行左键点击🖱️以添加拐角、选择项目、完成操作等 |
