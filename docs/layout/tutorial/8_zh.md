...
动态布线编辑器(DRE)是另一个强大的[交互式布线](/router/guide/19_zh.md#4)功能。与手动布线编辑器需要指定每个走线拐角不同，您只需开始走线并沿所需方向移动鼠标，走线拐角会随着鼠标移动动态添加。
...
[交互式布线](/router/guide/19_zh.md#4)功能允许您通过将走线模式压缩为多走线总线模式，同时布线大型多位数据总线的所有网络成员，同时保持所有设计规则。这减少了设计时间并优化了 PCB 空间使用。总线布线使用 DRE 选项，其中走线和过孔会移动为新布线腾出空间，而不会产生间距违规。
