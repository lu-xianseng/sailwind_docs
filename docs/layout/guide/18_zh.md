...
属性层级的级别是对象相关的，即每个对象具有不同的层级。您可以使用[属性特性对话框](/logic/guide/24_zh.md#23)中的“对象”选项卡修改每个属性的层级。下表显示了每种对象类型的属性默认层级。
...
尽管您可以使用[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)（选择对象 > **右键**单击 > **属性**弹出菜单项）向设计对象添加新属性，但必须使用属性辞典来设置属性值的属性。
...
3. 在[属性特性对话框](/logic/guide/24_zh.md#23)，在属性文本框中输入要创建的新属性名称
...
8. 点击**确定**关闭[属性特性对话框](/logic/guide/24_zh.md#23)
...
6. 点击**确定**关闭[属性特性对话框](/logic/guide/24_zh.md#23)
...
3. 在[属性特性对话框](/logic/guide/24_zh.md#23)输入新属性名称
...
4. 在**类型**标签页选择测量类型并执行以下操作：
   ...
   - 在单位框输入单位名称
     ...
     > [!tip]
     >
     > 如果使用单位 Mil，请在电路板级别分配属性，否则当您更改设计单位时(使用[选项对话框](/logic/guide/24_zh.md#78)>**全局**类别>常规子类别)，单位不会改变。
...
7. 从列表中选择新测量单位或输入新单位
   ...
   > [!tip] 必须为属性指定测量值才能保存并关闭[属性特性对话框](/logic/guide/24_zh.md#23)。
...
9. 点击**确定**接受限制并关闭[属性特性对话框](/logic/guide/24_zh.md#23)
...
2. 在[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)中从列表中选择要修改的属性。默认属性和设计属性会列出。可以从组列表中选择属性组来筛选视图，仅显示特定组的属性。如果属性是结构化属性，则会分组。
   ...
   > [!tip] 如果选择显示隐藏复选框，可以查看没有可见属性的属性组。可以在[属性特性对话框](/logic/guide/24_zh.md#23)的**对象**标签页设置属性是否隐藏。
...
4. 在[属性特性对话框](/logic/guide/24_zh.md#23)点击**类型**标签页，然后点击属性的类型。

   > [!danger]
   >
   > 如果使用设计单位 Mil，请在电路板级别分配属性，否则当您更改设计单位时(使用[选项对话框](/logic/guide/24_zh.md#78)>**全局**类别>常规子类别)，单位不会改变。
...
只能为设计可用的属性[添加属性值](/logic/tutorial/14_zh.md#2-3)。要为设计对象创建新属性，请参阅"[为设计创建属性](#6)"。可以在值中包含单位。`SailWind Layout` 还提供一组默认单位(和单位前缀)作为输入和输出使用。更多信息请参阅"默认单位"。
...
4. 在单元格中输入值并按 Enter 键
   ...
   > [!tip] 只能为设计可用的属性[添加属性值](/logic/tutorial/14_zh.md#2-3)。要为设计对象创建新属性，请参阅"[为设计创建属性](#6)"。可以在值中包含单位。`SailWind Layout` 还提供一组默认单位(和单位前缀)作为输入和输出使用。更多信息请参阅"默认单位"。
...
您可以使用[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)修改对象或同类型多个对象的属性值。当[选择多个对象](/logic/guide/9_zh.md#3-2)修改属性时，“属性”列表将显示所有属性名称的并集。
...
3. 右键点击并选择 **属性** 弹出菜单项。“[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)”将出现。选定对象的属性信息将以电子表格形式显示在列表中。
...
> [!tip] 要快速为多个对象添加高度信息，请使用“通过[属性管理器](#9)为设计对象[添加属性值](/logic/tutorial/14_zh.md#2-3)”中的步骤。
...
1. 在设计中[选择一个对象](/logic/guide/9_zh.md#3-1)（例如组件、组件焊盘、网络或过孔），右键点击并选择 **属性** 弹出菜单项。
