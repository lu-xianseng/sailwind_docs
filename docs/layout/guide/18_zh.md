...
尽管您可以使用[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)（选择对象 > **右键**点击🖱️ > **属性**）向设计对象添加新属性，但必须使用属性辞典来设置属性值的属性。
...
2. 如果要编辑用于旧版文件的默认属性，点击🖱️**主页>导入**菜单项并导入文件 *Default.asc* 或*DefaultAttributeDictionary.asc*
...
6. 要覆盖现有文件，点击🖱️**主页>导出**菜单项。执行此操作可更改 *default.asc* 或*DefaultAttributeDictionary.asc* 文件。如果要创建仅用于新文件的启动文件，请转至步骤 10

7. 选择**ASCII**作为文件类型并点击🖱️**保存**。将显示 [ASCII 输出对话框](/layout/guide/Dialogs/46_zh.md#31)

8. 在 Sections 列表中，选中✅ Attributes 复选框

9. 点击🖱️**确定**。默认属性辞典将被替换
...
> [!tip] 使用非标准格式的值可能会导致 SailWind Layout 更改该值，即使您没有更改当前单位。例如，输入⌨️值 0001.2000mil 会导致 SailWind Layout 将值更改为 1.2mil。
...
> [!tip] 额外的逗号(,)意味着您可以输入⌨️不带前缀的单位缩写。例如，您可以在属性值中输入⌨️ O 表示欧姆；这是一个有效值。但是，您不能在属性值中添加 F 表示法拉；这不是有效值。您必须使用带前缀的法拉。
...
3. 通过键入⌨️标题[SI Units]添加新的属性单位部分
...
> [!tip] 您可以在等号(=)后列出有效前缀，这些前缀将同时用于输入⌨️和输出。
...
2. 在[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)中，点击🖱️**新建**

3. 在属性特性对话框，在属性文本框中输入⌨️要创建的新属性名称
...
5. 在列表框中输入⌨️属性值，点击🖱️**设置**将项目添加到列表
...
9. 点击🖱️**关闭**关闭[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)

👀‍**结果**

列表框中包含您输入⌨️的属性可能值。列表中的项目显示为列表，可以在对象属性或[属性管理器对话框](/layout/guide/Dialogs/46_zh.md#44)中选择值。

##### 删除列表条目

点击🖱️**清除**或**全部清除**删除属性项目。
...
5. 点击🖱️**清除**或**全部清除**删除项目

6. 点击🖱️**确定**关闭属性特性对话框

7. 点击🖱️ **关闭** 关闭[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)
...
2. 在[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)中点击🖱️**新建**
...
5. 点击🖱️**清除**或**清除用户**删除项目

6. 默认单位保留在列表中

7. 从列表中选择新测量单位或输入⌨️新单位
...
9. 点击🖱️**确定**接受限制并关闭属性特性对话框

10. 点击🖱️**关闭**关闭[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)
...
6. 完成属性特性设置后，点击🖱️**确定**返回[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)
...
4. 在属性特性对话框点击🖱️**类型**标签页，然后点击🖱️属性的类型。

   > [!danger]
   >
   > 如果使用设计单位 Mil，请在电路板级别分配属性，否则当您更改设计单位时(使用选项对话框>**全局**类别>常规子类别)，单位不会改变。

5. 点击🖱️对象标签页，点击🖱️要限制的对象。不能将属性分配给受限对象

6. 要使用默认层次结构，点击🖱️使用默认层次结构复选框

7. 如果要启用属性的 ECO 注册，点击🖱️ **ECO已注册**复选框

8. 完成属性特性设置后点击🖱️**确定**返回[属性辞典对话框](/layout/guide/Dialogs/46_zh.md#43)
...
1. 点击🖱️**编辑 > 属性管理器**菜单项
2. 在[属性管理器对话框](/layout/guide/Dialogs/46_zh.md#44)的查看区域点击🖱️视图选项：
   - **已选定** 列出设计中选定对象的属性
...
2. 在[属性管理器对话框](/layout/guide/Dialogs/46_zh.md#44)中点击🖱️**显示**
...
只能为设计可用的属性添加属性值。要为设计对象创建新属性，请参阅"[为设计创建属性](#6)"。可以在值中包含单位。SailWind Layout 还提供一组默认单位(和单位前缀)作为输入⌨️和输出使用。更多信息请参阅"默认单位"。
...
4. 在单元格中输入⌨️值并按 Enter 键

   > [!tip] 只能为设计可用的属性添加属性值。要为设计对象创建新属性，请参阅"[为设计创建属性](#6)"。可以在值中包含单位。SailWind Layout 还提供一组默认单位(和单位前缀)作为输入⌨️和输出使用。更多信息请参阅"默认单位"。
...
4. 在单元格中输入⌨️值并按 Enter 键
   ...
   > [!tip] 可以在值中包含单位。SailWind Layout 还提供一组默认单位(和单位前缀)作为输入⌨️和输出使用。更多信息请参阅"默认单位"。
...
2. 在[属性管理器对话框](/layout/guide/Dialogs/46_zh.md#44)中，点击🖱️要应用到其他对象的属性值所在的单元格。
...
2. 在[属性管理器对话框](/layout/guide/Dialogs/46_zh.md#44)中，点击🖱️ **显示**。

   > [!warning] 属性必须在[属性管理器](#9)中被选中✅才能查看。更多信息，请参阅“选择属性以在[属性管理器](#9)中列出”。

3. 在“[显示属性对话框](/layout/guide/Dialogs/54_zh.md#27)”中，从属性列表中选择一个属性。

4. 为每种属性类型选择要创建的摘要类型的复选框。

5. 点击🖱️ **确定** 返回[属性管理器对话框](/layout/guide/Dialogs/46_zh.md#44)。
...
3. 从“组”列表中点击🖱️要查看的属性组。如果分配了多个属性且其中一些是结构化属性，此列表将作为过滤器，允许您选择要查看的属性组。
...
6. 从“属性”列表中选择一个属性，或在空白单元格中输入⌨️新属性的名称。无法添加隐藏或只读属性。属性名称最长可为 255 个字符。可以使用任何可打印字符（包括空格）作为属性名称，但空格不能作为名称的第一个或最后一个字符。属性名称不区分大小写，且在整个设计中定义，而非按对象定义。

   > [!tip] 当向设计添加新属性时，它也会被添加到属性辞典中。

7. 双击“值”列中的空白单元格（位于刚添加的属性旁边）。为属性分配值。可以在值中包含单位。SailWind Layout 还提供了一组默认的单位（和单位前缀），可用于输入⌨️和输出。更多信息，请参阅“默认单位”。

   > [!tip] 如果使用设计单位 Mil，请在板级分配属性。否则，当更改设计单位时（使用“选项”对话框的“全局”选项卡），单位不会更改。

8. 点击🖱️ **关闭** 以关闭[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)。
...
1. 如果属性为 ECO 注册，请进入 ECO 模式。点击🖱️ [ECO 工具栏](/layout/guide/36_zh.md#2)按钮进入 ECO 模式。如果未先进入 ECO 模式，系统将显示消息提示您需要进入 ECO 模式。
...
4. 从“组”列表中点击🖱️要查看的属性组。如果分配了多个属性且其中一些是结构化属性，此列表将作为过滤器，允许您选择要查看的属性组。

5. 从“属性用于”列表中，点击🖱️要分配属性的属性层级。层级会根据步骤 1 中选择的对象而变化。更多信息，请参阅“属性层级”。

   > [!warning] 
   > 如果选择了多个对象，则无法点击🖱️层级。属性将在当前层级分配；例如，如果选择了多个元件，属性将在组件层级分配。

6. 点击🖱️要修改的属性值的单元格（位于“值”列中）。如果单元格为空，则表示该属性已分配给对象但没有值，也可能表示选定对象的属性值不同。

7. 点击🖱️ **编辑**。

8. 输入⌨️或点击🖱️新的属性值，然后按 Enter 键。新值将添加到对象中。更多信息，请参阅“属性值”。
...
1. 如果属性为 ECO 注册，请进入 ECO 模式。
   ...
   点击🖱️ **[ECO 工具栏](/layout/guide/36_zh.md#2)** 按钮进入 ECO 模式。[ECO 选项对话框](/layout/guide/Dialogs/49_zh.md#1)将出现。如果未先进入 ECO 模式，系统将显示消息提示您必须进入 ECO 模式。

   > [!tip] 无法编辑只读属性。

2. 选择要编辑的对象。

3. 右键点击🖱️并选择 **属性**。“[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)”将出现。选定对象的属性信息将以电子表格形式显示在列表中。

4. 从“组”列表中点击🖱️要查看的属性组。如果分配了多个属性且其中一些是结构化属性，此列表将作为过滤器，允许您选择要查看的属性组。

5. 点击🖱️要[删除属性值](#9-5)的单元格。
...
1. 如果属性为 ECO 注册，请进入 ECO 模式。点击🖱️ **[ECO 工具栏](/layout/guide/36_zh.md#2)** 按钮进入 ECO 模式。[ECO 选项对话框](/layout/guide/Dialogs/49_zh.md#1)将出现。如果未先进入 ECO 模式，系统将显示消息提示您必须进入 ECO 模式。
...
3. 在[对象属性对话框](/layout/guide/Dialogs/51_zh.md#31)中，点击🖱️ **添加** 按钮。

   > [!warning] 某些属性要求您处于 ECO 模式。

4. 输入⌨️ Geometry.Height，或从列表中选择它。
...
1. 在设计中选择一个对象（例如组件、组件焊盘、网络或过孔），右键点击🖱️并选择 **属性**。

2. 在“属性用于”列表中，点击🖱️ PCB。将显示 PCB 在属性层级的 PCB 级别的属性。

3. 点击🖱️ **添加** 按钮以添加新属性。

4. 在新行中，输入⌨️新属性，或使用列表选择预定义的属性。
