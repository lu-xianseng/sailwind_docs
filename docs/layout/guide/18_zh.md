...
3. 您可以将属性分配给多个类型的多个对象。有关更多信息，请参阅鼠标“属性管理器”主题。
...
2. 在属性辞典对话框中从列表中选择要修改的属性。默认属性和设计属性会列出。可以从组列表中选择属性组来筛选视图，仅显示特定组的属性。如果属性是结构化属性，则会分组。
...
2. 在属性辞典对话框从列表中选择要删除的属性。可以从组列表中选择属性组来筛选视图，仅显示特定组的属性。如果属性是结构化属性，则会分组。清除显示隐藏复选框可筛选列表，仅显示至少包含一个可见属性的属性组。
...
3. 在"显示属性对话框"，在组列表中选择属性组或 `<all>` 选择所有属性
...
> [!danger] 您无法删除只读属性，或在非 `ECO` 模式下删除 `ECO` 注册属性。
...
1. 如果属性为 `ECO` 注册，请进入 `ECO` 模式。点击 `ECO` 工具栏按钮进入 `ECO` 模式。如果未先进入 `ECO` 模式，系统将显示消息提示您需要进入 `ECO` 模式。
...
3. 右键点击并选择 **属性** 弹出菜单项。“对象属性对话框”将出现。选定对象的属性信息将以电子表格形式显示在列表中。
...
1. 如果属性为 `ECO` 注册，请进入 `ECO` 模式。点击 **`ECO` 工具栏** 按钮进入 `ECO` 模式。`ECO` 选项对话框将出现。如果未先进入 `ECO` 模式，系统将显示消息提示您必须进入 `ECO` 模式。
