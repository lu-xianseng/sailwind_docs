...
> 
> 要重新同步两个程序，如果 SailWind Router是活动程序，请单击 SailWind Router主工具栏上的布局按钮；如果 SailWind Layout是活动程序，请单击 SailWind Layout主工具栏上的布线按钮。
...
2. 在[选项对话框](/logic/guide/24_zh.md#78)中，单击鼠标同步类别

3. 在"布局和布线器同步"区域，单击"启用"

4. 切换到 SailWind Router会自动将您的设计置于 SailWind Layout的 `DRC` 关闭模式；SailWind 布线器中的 `DRC` 模式不受影响

   要恢复 `DRC` 模式，当您切换回 SailWind Layout时(切换到 SailWind Router之前存在的设置)，单击"返回时恢复 `DRC` 模式"。根据设计的大小，恢复 `DRC` 模式可能需要几分钟。
...
2. 使用以下任一方式切换到 SailWind Router：
  ...
  - 单击**工具>布线器**菜单项，在 [SailWind Router链接对话框](/layout/guide/Dialogs/52_zh.md#5)中选择一个操作并单击**继续**
...
如果执行了不支持的命令或更改了不支持的选项，非活动程序将失去同步。要重新同步两个程序，如果 SailWind Router是活动程序，请单击 SailWind Router主工具栏上的布局按钮；如果 SailWind Layout是活动程序，请单击 SailWind Layout主工具栏上的布线按钮。
...
如果您单击"在后台运行"，SailWind 布线器监视器会出现，但 SailWind Router不会出现在桌面上。

如果您单击"在前台运行"，SailWind 布线器监视器会出现。SailWind 布线器也会打开并自动为当前 SailWind Layout[设计自动布线](/router/tutorial/7_zh.md#6)。
...
2. 在 [SailWind Router链接对话框](/layout/guide/Dialogs/52_zh.md#5)中，在"布线策略"区域，单击**设置**
