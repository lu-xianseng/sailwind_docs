...
- 元件类型表示元件的电气配置，包括其逻辑系列、管脚、门和门管脚，以及门管脚到元件管脚的映射。它还包括其他信息，如元件属性和分配给元件的 PCB 封装列表。将元件添加到设计时，它作为元件类型的实例添加。
  ...
  您可以通过在[元件信息对话框](/logic/guide/24_zh.md#85)的选项卡中指定元件属性来创建新的 SailWind 元件类型，或修改现有元件类型，如下所示：
...
- 连接器在 `SailWind Logic` 中用作连接器的默认和替代 [CAE 封装](/logic/guide/7_zh.md#7)。当常规选项卡上的连接器复选框被清除，或在门选项卡上为元件分配了门时，此选项卡不可用。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)的常规选项卡上，选择逻辑系列；这将设置参考标识符前缀。
...
10. 单击**确定**。有关说明，请参见"[元件类型错误检查](#8-6)"。

11. 在[将元件类型保存到库对话框](/logic/guide/24_zh.md#118)中，键入新元件的名称，然后单击**确定**。
...
8. 单击**确定**。更多信息请参见"[元件类型错误检查](#8-6)"。

9. 在[将元件类型保存到库对话框](/logic/guide/24_zh.md#118)中，键入新元件的名称，然后单击**确定**。
...
11. 单击**确定**。更多信息请参见"[元件类型错误检查](#8-6)"。

12. 在[将元件类型保存到库对话框](/logic/guide/24_zh.md#118)中，键入新元件的名称，然后单击**确定**。
...
2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，选择一个库，然后单击**元件**按钮。
...
5. 要分配一个或多个封装：
    ...
    - 要分配库中不存在的封装，请单击**分配新**。在[分配新 PCB 封装对话框](/layout/guide/Dialogs/46_zh.md#40)中，键入名称，然后单击**确定**。
...
键入 [CAE 封装](/logic/guide/7_zh.md#7)的名称或使用"为门分配封装"对话框为门分配默认和替代 [CAE 封装](/logic/guide/7_zh.md#7)。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)中，选择门选项卡。

5. 选择 [CAE 封装](/logic/guide/7_zh.md#7) 1 下的单元格，然后单击**编辑**按钮或双击 [CAE 封装](/logic/guide/7_zh.md#7) 1 下的单元格。

6. 在 [CAE 封装](/logic/guide/7_zh.md#7)框中键入封装名称，或单击（浏览）按钮从库中搜索封装。
...
8. 要分配一个或多个封装：
    ...
    - 您最多可以为元件分配四个 [CAE 封装](/logic/guide/7_zh.md#7)。分配的封装必须具有相同数量的管脚。
...
对于每个门，您可以键入 [CAE 封装](/logic/guide/7_zh.md#7)名称；用于在原理图中显示元件的逻辑符号名称。替代封装分配必须具有相同数量的管脚。您可以为每个门定义一个主要和三个替代封装。
...
管脚表包含管脚数据的映射，并允许您添加、编辑和[重新编号管脚](/logic/guide/7_zh.md#8-7-6)。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)中，选择管脚选项卡。
...
12. 单击**确定**关闭[元件信息对话框](/logic/guide/24_zh.md#85)。
...
10. 单击序列（Seq.）单元格并键入门序列号。序列号确定 CAE 门管脚到 PCB 封装管脚的映射。序列自动与替代 [CAE 封装](/logic/guide/7_zh.md#7)共享。例如，它显示 CAE 门封装上的管脚编号如何出现；因此，在门 A 中，序列号 1 可能是管脚 1，但在门 B 中，序列号 1 将是管脚 4。
...
6. 单击**重新编号**按钮。在"[重新编号管脚](/logic/guide/7_zh.md#8-7-6)"对话框中，管脚数量框显示选择重新编号的管脚数量。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)中，单击属性选项卡。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)中，单击连接器选项卡。

5. 要过滤门封装列表，请在库列表中选择一个库，在过滤框中键入通配符，然后单击**应用**。要显示所有项目，请键入星号\*并单击**应用**。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)中，选择管脚映射选项卡。
...
6. 使用以下方法之一映射管脚：
    ...
    - **使用未映射管脚列表** 在未映射管脚列表中，选择一个或多个字母数字。如果您有要映射的连续列表，请在元件类型列中选择一个或起始行。单击**映射**。
      ...
      > [!danger]       如果管脚编号有任何错误，请通过编辑**管脚**选项卡上的表进行更正。不要尝试在**管脚映射**选项卡上的表中[重新编号管脚](/logic/guide/7_zh.md#8-7-6)。
