...
| OLE  | .ole | 可使用编辑 > 插入新对象菜单项将其他应用程序的文件作为OLE 对象嵌入到设计中如果将这些对象导出到 OLE 文件，可使用文件 > 导入菜单项将其作为集合嵌入到另一个设计中更多信息请参阅"在 SailWind Layout 中插入 OLE 对象" |
...
|  |  | 设计中的虚拟过孔将作为过孔导出 |
|  |  | 更多信息请参阅"中间数据格式" |
| OLE  | .ole | 可使用编辑 > 插入新对象菜单项将其他应用程序的文件作为OLE 对象嵌入到设计中如果将这些对象导出到 OLE 文件，可使用文件 > 导入菜单项将其作为集合嵌入到另一个设计中 |
...
1. 点击🖱️**主页 > 导出**菜单项。保存为 DXF 类型并在[导出对话框](/layout/guide/Dialogs/49_zh.md#11)中点击🖱️**设置**。
...
1. 点击🖱️**工具 > [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)**菜单项。
