...
- 从 PADS 9.0 开始，芯片元件和倒装芯片不再由其系列名称(DIE 或 FLP)标识，而是由[元件信息对话框](/logic/guide/24_zh.md#85)常规选项卡中的特殊用途设置标识。当您导入由 PADS 9.0 之前版本创建的 ASCII 文件时，这些特殊用途设置会自动设置为具有 DIE 或 FLP 逻辑系列的元件。元件的系列名称保持不变。
...
- 从 PADS 9.0 开始，芯片元件和倒装芯片不再由其系列名称(DIE 或 FLP)标识，而是由[元件信息对话框](/logic/guide/24_zh.md#85)常规选项卡中的特殊用途设置标识。当您将设计导出到 PADS 9.0 之前版本的 ASCII 文件时，会发生以下变化：
...
4. 在 [ASCII 输出对话框](/layout/guide/Dialogs/46_zh.md#31)中，勾选适当的复选框以指定要写入 ASCII 文件的设计部分，或点击**全选**。
...
8. 如果要导出标准文件，并且想要定义 DXF 钻孔尺寸和符号，请执行以下步骤：
     ...
     b. 在[设置 DXF 钻孔尺寸和符号对话框](/layout/guide/Dialogs/54_zh.md#23)中，要为 2D 线库项指定钻孔尺寸，请点击**添加**，在钻孔尺寸框中键入值，然后在库等效框中键入库项名称。
...
1. 点击**主页 > 导出**菜单项。保存为 DXF 类型并在[导出对话框](/layout/guide/Dialogs/49_zh.md#11)中点击**设置**。
...
8. 如果任何导出的元件类型和封装对的 Geometry.Height 属性不存在或设置为零，则每个对都会出现缺少高度对话框。如果要在导出元件类型和封装对之前指定高度信息，请在高度框中键入高度。
   ...
   如果不指定高度(或将其指定为零)，机械设计系统在[导入 IDF 文件](#11)时可能会提示您输入高度。
...
1. 点击**工具 > [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)**菜单项。
...
4. 在[元件信息对话框](/logic/guide/24_zh.md#85)的**属性**选项卡中，添加 HOLE 元件属性。
