...
更改导线宽度后，移动鼠标。注意新线段现在如何以新指定的宽度添加。
...
1. 开始交互式布线，当导线段附着到鼠标时，右键点击，指向 **End Via Mode**，然后点击 **End Via**。勾选标记指示当前模式
...
动态布线是另一个强大的交互式布线功能。您不需要指示每个导线拐角，只需开始布线并朝希望导线流动的方向移动鼠标。导线拐角在您移动鼠标时动态添加。
...
4. 在电路板上垂直移动鼠标，朝连接目的地的大致方向。注意使用动态交互式布线如何自动选择绕过障碍物的路径并创建导线模式
