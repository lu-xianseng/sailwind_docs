...
若不超过两个管脚对共享导线段，SailWind Router 会自动建立规则边界使设计符合程序规则。[鼠标"规则边界"](#page-3-1)改进了此类冲突管理，消除了打开文件时需断开导线的要求。
...
- 无论 SailWind Layout 中设置如何，[设计规则检查](/layout/guide/19_zh.md#1-6)（DRC）都将启用

- 内联导线拐角将被移除

有关打开文件的更多详细信息及打开过程中发生的处理，请参阅[鼠标"关于打开文件"](#page-1-0)。
...
2. 在[选项对话框](/logic/guide/24_zh.md#78)的全局类别备份子类别中，在间隔框中输入备份间隔时间（分钟）
...
5. 勾选"在设计目录中[创建备份文件](/layout/guide/17_zh.md#1)"复选框，将备份文件置于设计所在目录

6. 点击**确定**

👀‍**结果**

备份将根据本流程中的设置保存，可保存在设计目录或"[选项对话框](/logic/guide/24_zh.md#78)全局类别文件位置子类别"指定的默认备份位置。同时，已保存备份的链接将记录在输出窗口**状态**选项卡显示的会话日志中。
...
3. 选择要归档的文件和文件夹（参见鼠标"[归档器对话框](/layout/guide/Dialogs/46_zh.md#27)"）
