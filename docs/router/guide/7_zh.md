...
3. 设置设计对象的颜色：
   ...
   > [!tip] 要更改可选颜色的调色板，请参阅"[更改颜色](#page-2-0) [调色板](#page-2-0)"
...
您可以保存设计工作中常用的显示颜色设置。参见"[保存配色](#page-2-1) [方案](#page-2-1)"。
...
您可以使用"[选项对话框，全局类别，常规子类别](/layout/guide/Dialogs/51_zh.md#44)"中的"通过以下方式区分高亮对象"列表设置高亮显示方式。
...
使用调色板以特定颜色显示选定的网络。当您[为网络分配颜色](/layout/guide/32_zh.md#5-1)时，所有层上的焊盘、过孔和连接都会以指定颜色显示。这在分割平面时更容易识别网络。您所做的任何网络颜色分配都会在保存设计时一起保存。

## 平移、缩放和滚动显示 \{#6}

使用鼠标滚轮可以滚动、[平移和缩放](/layout/tutorial/1_zh.md#5)工作区显示。
