...
- **用户**：可以通过输入⌨️单个尺寸的值来指定直径尺寸。也可以通过输入⌨️两个值（第一个为长度，第二个为宽度，用逗号分隔）来指定方形或矩形尺寸。
...
为特定应用选择合适的过孔需要考虑多个因素：所需的设计过孔间距格栅、走线和间距宽度、需要在过孔之间通过的走线数量、材料厚度以及过孔可能需要承载的功率。

[过孔计算器](#3)旨在接受前述部分因素作为用户输入⌨️，并生成其余因素作为输出，同时计算结果过孔的合适焊盘栈。
...
- **焊盘栈信息**：通过更改输入⌨️并评估输出，可以找到大多数应用的合适过孔。然后使用焊盘栈信息在设计数据库中定义过孔。
...
1. 在主工具栏上，点击🖱️**计算**按钮，然后点击🖱️[过孔计算器](#3)。

2. 在“[过孔计算器](#3)”中，如果应用是针对 BGA 的，首先使用 BGA 计算器选择适合 BGA 的过孔。

3. 根据设计约束调整走线数量、走线和间距宽度以及格栅，然后点击🖱️**计算**按钮计算并显示结果。
