...
焊盘图形向导用于为您的 `CAD` 工具生成焊盘图形。选择特定元件类型，输入⌨️元件尺寸，指定要使用的规则文件，即可生成可直接用于 `CAD` 工具的定制焊盘图形。
...
| 自动导入区域                            | 如果您希望自动导入输出，请勾选✅元件类型、封装或两者。输入⌨️您希望导入新元件的 SailWind Layout库名称。                                                                       |
...
设置选项后，点击🖱️**创建**或**创建并关闭**按钮开始转换过程。
...
1. 选择以下之一：
    ...
    - 使用库编辑器中的多元件向导：
        ...
        iii. 然后右键点击🖱️**工具 > 向导**或从菜单栏点击🖱️启动"[多元件向导对话框](/lpcreator/guide/4_zh.md#9)"。

    iv. 提供所需设置并点击🖱️**继续**启动向导。
...
1. 选择以下之一：
	...
	- 使用库编辑器中的多元件向导：
		...
		iii. 然后右键点击🖱️**工具 > 向导**或从菜单栏点击🖱️**工具 > 向导**启动"[多元件向导对话框](/lpcreator/guide/4_zh.md#9)"。
...
5. 根据需要点击🖱️**创建**或**创建并关闭**。

6. 在 SailWind Layout中，在新的空白设计中，点击🖱️**文件 > 导入 > ASCII**菜单项并将*.asc*文件导入设计。

7. 一旦库元件导入到空白设计中，右键点击🖱️**选择组件**。元件被导入，堆叠在原点，就像导入网表时一样。

8. 选择组件或右键点击🖱️**全选**。

9. 右键点击🖱️**保存到库**。

10. 在"保存元件类型和封装到库"对话框中，为元件类型和封装都点击🖱️**全选**。

11. **选择**所需的保存到库位置并点击🖱️**确定**。
