...
def process_file(file_path, all_headings):
    """处理单个文件，添加标题链接"""
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
        # 检查当前文件是否需要处理
    ...
    # 跳过标题行本身，用正则表达式匹配任意等级的标题，无论是否存在换行符
    skip_positions = []
    ...
    sorted_headings = sorted(
        [(text, anchor_id, file)
         ...
         if len(text.strip())>= 4], # 限制一下，超过三个字的标题就不做匹配了
    ...
    for match in pattern.finditer(content):
        ...
        if text in text_mapping:
            ...
            if current_file_headings:
                ...
                base_prefix = r'D:\downloads\sailwind_docs\docs'  # 注意使用原始字符串
...
def main():
    ...
    # 顺序处理每个文件
    for file_path in all_headings.keys():
        new_content = process_file(file_path, all_headings)
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
